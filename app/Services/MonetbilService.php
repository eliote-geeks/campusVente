...
class MonetbilService
{
    ...
    /**
     * Initier un paiement simplifié pour Premium Dating (basé sur les fichiers de référence)
     */
    public function initiatePremiumDatingPayment($params)
    {
        try {
            $cleanPhone = $this->cleanPhoneNumber($params['phone']);
            
            // Paramètres simplifiés comme dans les fichiers de référence
            $paymentParams = [
                'amount' => 1000, // 1000 FCFA fixe pour Premium Dating
                'phone' => $cleanPhone,
                'email' => $params['email'],
                'country' => 'CM',
                'currency' => 'XAF',
                'item_ref' => $params['payment_ref'],
                'user' => $params['user_id'],
                'first_name' => $params['first_name'] ?? '',
                'last_name' => $params['last_name'] ?? '',
                'return_url' => url('/api/v1/monetbil/success'),
                'notify_url' => url('/api/v1/monetbil/notify'),
                'lang' => 'fr'
            ];
            
            // Générer l'URL avec la clé de service directement dans l'URL
            $queryString = http_build_query($paymentParams);
            $paymentUrl = "https://www.monetbil.com/widget/v2.1/{$this->serviceKey}?" . $queryString;
            
            Log::info('URL de paiement Premium Dating générée', [
                'url' => $paymentUrl,
